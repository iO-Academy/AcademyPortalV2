...
class AddApplicantActionController extends Controller
{
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        try {
            ...
            ApplicationValidator::checkExists($newApplicant['circumstance_id'],
                $this->model->getAllCircumstances()[$newApplicant['circumstance_id'] - 1], "Circumstance ID");
            ApplicationValidator::checkNumeric($newApplicant['circumstance_id'], "Circumstance ID");
            ApplicationValidator::checkExists($newApplicant['funding_id'],
                $this->model->getAllFundingOptions()[$newApplicant['funding_id'] - 1], "Funding ID");
            ApplicationValidator::checkNumeric($newApplicant['funding_id'], "Funding ID");
            ApplicationValidator::checkExists($newApplicant['cohort_id'],
                $this->model->getAllCohorts()[$newApplicant['cohort_id'] - 1], "Cohort ID");
            ApplicationValidator::checkNumeric($newApplicant['cohort_id'], "Cohort ID");
            ApplicationValidator::checkExists($newApplicant['heard_about_id'],
                $this->model->getAllHearAboutUs()[$newApplicant['heard_about_id'] - 1], "Heard About ID");
